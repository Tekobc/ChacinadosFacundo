...
export const recepcionSchema = z
  .object({
    ...
    cantidad: z.number({ invalid_type_error: 'Ingresá una cantidad válida' }).positive('La cantidad debe ser mayor a 0'),
    unidad: z.string().min(1, 'La unidad es obligatoria'),
    temperatura_recepcion: z.number({ invalid_type_error: 'Temperatura inválida' }).optional().nullable(),
...
export const parteProduccionSchema = z.object({
  ...
  cantidad_producida: z.number({ invalid_type_error: 'Cantidad inválida' }).positive('La cantidad debe ser mayor a 0'),
  unidad: z.string().min(1, 'La unidad es obligatoria'),
  observaciones: z.string().optional(),
